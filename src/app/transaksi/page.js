...
// Helper function untuk mendapatkan tanggal & jam default
...
export default function Home() {
    ...
    const initialFormState = { judul: '', nominal: '', tanggal: defaultDate, jam: defaultTime, catatan: '', bukti_url: '', kategori_id: '' };
    ...
    const fetchTransaksi = async () => {
        try {
            ...
            if (!res.ok) {
                if (res.status === 401) setTransaksi([]);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const nominalToSubmit = tipeKategori.toUpperCase() === 'PENGELUARAN'
    ...
    return (
        <main className="container mx-auto p-4 md:p-8 font-sans">
            ...
            <div className="flex justify-center gap-4 mb-8">
                <Link href="/dashboard" className="bg-gray-500 text-white px-5 py-2 rounded-md hover:bg-gray-600 font-semibold shadow-sm">
            ...
            <div className="bg-white p-6 rounded-lg shadow-md mb-8 max-w-2xl mx-auto">
                ...
                <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <input type="number" name="nominal" value={formData.nominal} onChange={handleInputChange} placeholder="Nominal" required className="p-3 border rounded-md" min="0" />
                    ...
                    <div className="md:col-span-2">
                        ...
                        <select id="kategori_id" name="kategori_id" value={formData.kategori_id} onChange={handleInputChange} required className="mt-1 block w-full p-3 border border-gray-300 rounded-md shadow-sm">
                            <option value="" disabled>-- Pilih Kategori --</option>
                            {kategoriList.map(k => (
                                <option key={k.id} value={k.id}>{k.name_kategori} ({k.tipe})</option>
                            ))}
                        </select>
                    </div>
                    ...
                    <input type="file" onChange={handleFileChange} disabled={isUploading} ref={fileInputRef} accept="image/png, image/jpeg" className="p-3 border rounded-md md:col-span-2" />
                    {isUploading && <p>Mengupload...</p>}
                    {buktiUrl && <p>Upload sukses: <a href={buktiUrl} target="_blank" rel="noopener noreferrer" className="text-blue-500 hover:underline">Lihat Bukti</a></p>}
                    <div className="md:col-span-2 flex items-center gap-4 mt-2">
                        <button type="submit" disabled={isUploading} className="bg-blue-600 text-white px-5 py-2 rounded-md hover:bg-blue-700 font-semibold disabled:bg-gray-400">
                            {editingId ? 'Update Transaksi' : 'Simpan Transaksi'}
                        </button>
                        {editingId && (
                            <button type="button" onClick={cancelEdit} className="bg-gray-500 text-white px-5 py-2 rounded-md hover:bg-gray-600 font-semibold">Batal</button>
                        )}
                    </div>
                </form>
            </div>
            ...
            <div className="bg-white p-4 md:p-6 rounded-lg shadow-md">
                ...
                <div className="flex justify-center gap-2 mb-4">
                    <button onClick={() => setFilter('semua')} className={`px-4 py-2 text-sm font-medium rounded-lg ${filter === 'semua' ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-700'}`}>Semua</button>
                    <button onClick={() => setFilter('pemasukan')} className={`px-4 py-2 text-sm font-medium rounded-lg ${filter === 'pemasukan' ? 'bg-green-600 text-white' : 'bg-gray-200 text-gray-700'}`}>Pemasukan</button>
                    <button onClick={() => setFilter('pengeluaran')} className={`px-4 py-2 text-sm font-medium rounded-lg ${filter === 'pengeluaran' ? 'bg-red-600 text-white' : 'bg-gray-200 text-gray-700'}`}>Pengeluaran</button>
                </div>
                {isLoading ? (
                        ...
                        <table className="w-full table-auto">
                            <thead className="bg-gray-100">
                                <tr>
                                    ...
                                    <th className="p-3 text-left font-semibold text-gray-600">Kategori</th>
                                    ...
                                    <th className="p-3 text-left font-semibold text-gray-600">Bukti</th>
                                ...
                                {filteredTransaksi.length > 0 ? (
                                    filteredTransaksi.map(t => {
                                        ...
                                        return (
                                            <tr key={t.id} className="border-b hover:bg-gray-50">
                                                ...
                                                <td className="p-3">
                                                    <p className="text-sm text-gray-800">{t.namaKategori}</p>
                                                </td>
                                                <td className={`p-3 font-bold ${isPengeluaran ? 'text-red-600' : 'text-green-600'}`}>
                                                    {formatRupiah(t.nominal)}
                                                </td>
                                                <td className="p-3 text-sm text-gray-700">
                                                    {new Date(t.tanggal).toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' })}
                                                    <span className="text-gray-500 block">{t.jam.slice(0, 5)}</span>
                                                </td>
                                                <td className="p-3">
                                                    {t.bukti_url ? (
                                                        <a
                                                ...
                                                <td className="p-3 flex gap-2 items-center">
                                                    <button onClick={() => handleEdit(t)} className="bg-yellow-400 text-white px-3 py-1 rounded text-sm hover:bg-yellow-500">Edit</button>
                                                    <button onClick={() => handleDelete(t.id)} className="bg-red-500 text-white px-3 py-1 rounded text-sm hover:bg-red-600">Hapus</button>
                                                </td>
                                            </tr>
                                        );
                                    })
                                    ...
                                    <tr>
                                        <td colSpan="5" className="text-center p-4 text-gray-500">
                                            {filter === 'semua' ? 'Belum ada transaksi.' : `Belum ada transaksi ${filter}.`}
                                        </td>
                                    </tr>
                                )}
                            </tbody>
