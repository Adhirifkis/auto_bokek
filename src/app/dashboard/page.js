...
const processDataForChart = (transactions) => {
  const grouped = transactions.reduce((acc, trx) => {
    ...
    const date = new Date(trx.tanggal).toLocaleDateString("id-ID", {
      day: "2-digit",
      month: "short",
    });
    if (!acc[date]) {
    ...
    <div className="bg-gray-50 min-h-screen font-sans">
      <main className="container mx-auto p-4 md:p-8">
        <div className="flex flex-wrap justify-between items-center gap-4 mb-8">
          ...
          <Link
            href="/transaksi"
            className="flex items-center gap-2 bg-blue-600 text-white px-5 py-3 rounded-lg hover:bg-blue-700 font-semibold shadow-md transition-transform transform hover:scale-105"
          >
                  ...
                  <li key={trx.id} className="flex items-center gap-4">
                    <div
                      className={`p-2 rounded-full ${
                        trx.kategori?.tipe.toUpperCase() === "PEMASUKAN"
                          ? "bg-green-100"
                          : "bg-red-100"
                      }`}
                    >
                    ...
                    <div className="flex-grow">
                      ...
                      <p className="text-sm text-gray-500">
                        {trx.kategori?.name_kategori || "N/A"}
                      </p>
                    </div>
                    <span
